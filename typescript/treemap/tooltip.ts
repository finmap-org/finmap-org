...
export class TooltipComponent {
  ...
  private updateTooltipContent(data: MarketData, node: any, currencySign: string): void {
    if (!this.element) return;
  ...
  private populateTooltipData(
    ...
    const position = this.element.querySelector('.tooltip-position') as HTMLElement;
    const marketcap = this.element.querySelector('.tooltip-marketcap') as HTMLElement;
    ...
    const exchange = this.element.querySelector('.tooltip-exchange') as HTMLElement;
    const industry = this.element.querySelector('.tooltip-industry') as HTMLElement;
    const sectorPercent = this.element.querySelector('.tooltip-sector-percent') as HTMLElement;
    const totalPercent = this.element.querySelector('.tooltip-total-percent') as HTMLElement;
    ...
    if (position) {
      const positionValue =
        node?.value !== undefined && data.isPortfolio ? node.value : data.positionValue || 0;
    ...
    if (volume) volume.textContent = `Volume: ${formatNumber(data.volume || 0)}`;
    if (trades) trades.textContent = `Trades: ${formatNumber(data.numTrades || 0)}`;
    if (exchange) exchange.textContent = `Exchange: ${data.exchange || 'N/A'}`;
    if (industry) industry.textContent = `Industry: ${data.industry || 'N/A'}`;
    if (sectorPercent) sectorPercent.textContent = `% of Sector: ${formatPercent(percentParent)}%`;
    if (totalPercent) totalPercent.textContent = `% of Total: ${formatPercent(percentRoot)}%`;
    if (items) items.textContent = `Items: ${formatNumber(data.nestedItemsCount || 0)}`;
  }
