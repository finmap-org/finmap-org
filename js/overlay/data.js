...
export async function fetchNews(ticker, companyName, date, signal) {
    ...
    try {
        ...
        newsItems.sort((a, b) => new Date(b.originalPubDate).getTime() - new Date(a.originalPubDate).getTime());
...
export async function fetchCompanyInfo(exchange, ticker, wikiPageIdEng, wikiPageIdOriginal, signal) {
    const config = getConfig();
    try {
        switch (exchange) {
            case 'nasdaq':
...
async function fetchHKCompanyInfo(ticker, language, signal) {
...
async function fetchWikiInfo(wikiPageIdEng, wikiPageIdOriginal, language, signal) {
